...
# pre-existing cross-module contract this extraction must not break.
from app.services.payment_lookup import AmbiguousOrderIdError as AmbiguousOrderIdError
...
def _find_payment(db: Session, order_id: str) -> Payment | None:
    ...
    gateway_order_id -- shared by every command that takes ORDER_ID, and
    with the admin bot's `/payment` lookup (see app.adminbot.queries).
    ...
    return find_payment_by_order_id(db, order_id)
