...
نسخهٔ فعلی برنامه **0.6.0-rc4** و head فعلی Alembic در این شاخه **0012** است.
...
centralpay review resolve-many ORDER_ID [ORDER_ID ...] \
    --resolution VALUE --note TEXT [--yes]   # فقط خطاهای تحویل
...
در production، `CENTRALPAY_UPDATE_REF` باید به‌طور عادی release tag معتبر مثل `v0.6.0-rc4` اشاره کند.
...
`0.6.0-rc4` هنوز pre-release است. برای تصمیم انتشار/استقرار به source فعلی، CI، `RELEASE_RISK_REGISTER.md` و چک‌لیست عملیاتی فعلی مراجعه کنید؛ نه به یک audit قدیمی.
