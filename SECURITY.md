...
- **one-time `centralpay update` bootstrap gap for the Caddy config sync itself (proven, not code-fixable in this commit):** `centralpay update` executes the ALREADY-INSTALLED `/usr/local/bin/centralpay` -- bash resolves every function body from that file when the process starts, and nothing that happens later in that same process (including `git checkout` replacing files on disk, or even `sync_management_wrapper` atomically replacing `/usr/local/bin/centralpay` itself near the end of the run) changes the already-parsed function bodies the running interpreter is executing. On the very first update that carries a host from a commit predating the Caddy-config-sync mechanism to one that includes it, the running (old) `perform_update` has no knowledge of `render-caddy-config.sh` and cannot call it -- not because of a bug, but because that call does not exist anywhere in the bytes the running process read at start. The OLD `perform_update` also contains no dynamic hook (no `source`d file, no git `post-checkout` hook, no hooks.d/run-parts directory) that the newly-checked-out commit could execute through instead; the only per-path invocation after checkout in every historical version is `compose build`/`compose up`, which never touches host-side scripts. This was audited directly against the pre-fix `perform_update` (candidates considered and rejected: self-`exec` of the running process, a staged/resume-token two-phase updater, a post-checkout hook point, an explicit wrapper handoff -- all require code that is, by definition, not yet installed on the host at the moment this transition runs). The new commit is still checked out and built/migrated/restarted normally; only the Caddyfile is not synced for that one transition. `sync_management_wrapper` installs the new script as `$MANAGEMENT_BIN` at the end of that same update, so the very NEXT `centralpay update` runs the real, current sync logic and closes the gap.
  - **`install.sh` reruns do NOT share this gap and are the recommended immediate remediation.** Unlike the installed `centralpay` wrapper, `install.sh` is never a locally cached copy: the documented install command (`curl -fsSL .../install.sh | sudo bash`, see [README.md](README.md)) downloads its CURRENT bytes fresh from `main` on every single invocation, so there is no stale-process problem to bootstrap past. `install.sh`'s `main()` unconditionally runs `fetch_repository` (refreshing the git checkout) and `sync_caddy_config` in BOTH the fresh-install and "keep existing configuration" paths, followed by a full `deploy_stack` (build + restart) -- so rerunning the installer once, choosing "keep existing configuration", closes the Caddy-sync gap (and fully redeploys the application) in a single invocation, even on a host that has never run `centralpay update` with the new code at all.
  - **Fail-visible backstop:** `centralpay status` and `centralpay diagnose` now include a read-only "Caddy configuration" check (`caddy_activation_status_line` in `scripts/centralpay`, backed by `render-caddy-config.sh --check` -- never mutates the Caddyfile, never validates via Docker, never writes the activation marker). Because `sync_management_wrapper` has already replaced the installed wrapper by the time the flawed first `update` finishes, the very next `status`/`diagnose` invocation already runs this check and prints a loud warning if the installed config is out of sync or its activation was never confirmed -- converting what was previously a silent, indefinitely-persisting gap into one surfaced immediately on the operator's next routine command, not just on the next `update`.
  - Operational mitigation, in order of preference: (1) rerun the installer once (`curl -fsSL https://raw.githubusercontent.com/Mhoseinshah1/centralpay-bridge/main/install.sh | sudo bash`, "keep existing configuration") -- single invocation, closes the gap immediately; (2) run `centralpay update` a second time (a no-op update against the same commit still runs the sync). A general self-relaunch mechanism to close this class of gap for all future `perform_update` changes, not just this one, remains a reasonable candidate for a dedicated follow-up; the risk of touching the core update/rollback execution model deserves its own focused review and is out of scope here
