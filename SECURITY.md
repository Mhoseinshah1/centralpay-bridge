...
- **one-time bootstrap gap for the Caddy config sync itself:** `centralpay update` executes the ALREADY-INSTALLED `/usr/local/bin/centralpay` -- bash resolves every function body from that file when the process starts, so the currently-running `perform_update` is whatever code was installed BEFORE this update began. On the very first update that carries a host from a commit predating the Caddy-config-sync mechanism to one that includes it, the running (old) `perform_update` has no knowledge of `render-caddy-config.sh` and does not call it; the new commit is checked out and built/migrated/restarted normally, but the Caddyfile is not synced for that one transition. `sync_management_wrapper` installs the new script as `$MANAGEMENT_BIN` at the end of that same update, so the very NEXT `centralpay update` (or an `install.sh` rerun) runs the real, current sync logic and closes the gap. This is an inherent property of a self-updating shell script with no relaunch/re-exec step -- not something a change shipped in the same commit that introduces new `perform_update` logic can close for itself, since a host that has not yet downloaded that commit cannot be running code from it. Operational mitigation: after upgrading a pre-existing installation past the commit that introduced `scripts/render-caddy-config.sh`, either run `centralpay update` a second time (a no-op update against the same commit still runs the sync) or rerun `install.sh` once (any answer, including "keep existing configuration") to force the Caddy config sync explicitly. A general self-relaunch mechanism to close this class of gap for all future `perform_update` changes, not just this one, is a reasonable candidate for a dedicated follow-up given the risk of touching the core update/rollback execution model deserves its own focused review
